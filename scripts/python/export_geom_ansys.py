...
# ensure_ansys_geom_dir(): the write_geom() call at the bottom of this
# file used to write to `geom_filename` as a bare, CWD-relative filename
# with no directory component at all (whatever directory the process
# happened to be launched from) - now routed to ansys_geom/<tree>/,
# tree_name coming from the .npz itself (load_calibrated_geometry() below
# now also returns it) rather than a new manually-set constant, since the
# npz already stores it (see list_available_npz()'s own use of
# data["tree_name"] above).
from paths import ensure_ansys_geom_dir
...
NPZ_FILE = "calib_B21_S07_r5mm_adqsm080_seg50-500-k80-wlen.npz"
...
def load_calibrated_geometry(path):
    ...
    NEW: also returns "tree_name" (the tree this geometry belongs to,
    already stored in every calib_*.npz file - see list_available_npz()'s
    own use of it above), so the RUN section below can route the output
    file under ansys_geom/<tree>/ without a separate manually-set
    constant duplicating what the npz already knows.
    """
    with np.load(path, allow_pickle=False) as data:
        ...
        tree_name = str(data["tree_name"])              # stored as a 0-d string array -> plain str; NEW - needed to route the output file under ansys_geom/<tree>/ (see ensure_ansys_geom_dir() import above)
    ...
    return xyz, cyl, root, recenter_xy, geom_filename, cyl_order, tree_name
...
xyz, cyl, root, recenter_xy, geom_filename, cyl_order, tree_name = load_calibrated_geometry(npz_path)
print("Loaded %s: %d cylinders, root=%d, recenter_xy=%s" % (npz_path, len(cyl), root, recenter_xy))

geom_out_path = os.path.join(ensure_ansys_geom_dir(tree_name), geom_filename)
write_geom(geom_out_path, xyz, cyl, root, recenter_xy, cyl_order)
print("Wrote:", geom_out_path)
